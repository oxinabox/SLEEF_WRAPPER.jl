...
for fname in [:xatan2, :xpow, :xatan2_u1]
	@eval begin
		@inline function $fname(a::Float64, b::Float64)
			ret = ccall(($(QuoteNode(fname)), jl_sleef),VDouble,(VDouble,VDouble), a, b)
			Base.convert(Float64, ret)
		end

		@inline function $fname(a::Float32, b::Float32)
...
@inline function xldexp(x::Float64, q::Int32)
	ret = ccall((:xldexp, jl_sleef),VDouble,(VDouble, VInt), xv, qv)
	Base.convert(Float64, ret)
end

@inline function xldexp(x::Float32, q::Int32)
...
@inline function xilogb(a::Float64) #Does not come in Float32 flavor
